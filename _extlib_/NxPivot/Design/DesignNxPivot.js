if (nexacro.NxPivot) {
	...
	_pNxPivot._setContents = function(str)
	{
		...
		var sXML = "";
		//alert(JSON.stringify(jsonContents, null, "\t"));
		
		
		
		var panel = this.jsonContents.panel;
		...
		if(this.usetoolbar==true)nHeight += nexacro.toNumber(toolbar.size);
		if(this.useitems==true)nHeight += nexacro.toNumber(items.size);
