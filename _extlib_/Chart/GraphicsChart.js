...
if (!nexacro._ChartGraphicsLib) {
	...
	nexacro._ChartGraphicsLib.isObject = function (value) {
		...
		if (nexacro._ChartGraphicsLib.isXComponent(value)) {
			return false;
		}
	...
	nexacro._ChartGraphicsLib.getUniqueId = function (prefix, separator) {
		if (nexacro._ChartGraphicsLib.isEmpty(prefix)) {
			prefix = "";
		}
		...
		var charcode = nexacro._ChartGraphicsLib._ALPHA_CHAR_CODES, math = Math;
		var seq = 0;
		var seq0;
	...
	if (nexacro._Browser == "Runtime") {
		...
		nexacro._ChartGraphicsLib._getLines = function (nexacroFont, txt, wordwrap, maxWidth) {
			var canvas = nexacro._ChartGraphicsLib._getDefaultCanvas(), ctx = canvas.getContext('2d');
			var lines, sz, i, len;
...
if (!nexacro._ChartGraphicsHashMap) {
	...
	nexacro._ChartGraphicsHashMap.makeHashKeyForArray = function (arr, toStringFn_) {
		var getUniFn = nexacro._ChartGraphicsLib.getUniqueId, i, len = arr.length, toStringFn = toStringFn_ || this._simpleToString;
...
if (!nexacro._ChartGraphicsMatrix) {
	...
	_pGraphicsMatrix.toString = function () {
		var f0 = [nexacro._ChartGraphicsClassUtils.fmtNumber(this._scaleX), nexacro._ChartGraphicsClassUtils.fmtNumber(this._shearX), nexacro._ChartGraphicsClassUtils.fmtNumber(this._translateX)];
		var f1 = [nexacro._ChartGraphicsClassUtils.fmtNumber(this._shearY), nexacro._ChartGraphicsClassUtils.fmtNumber(this._scaleY), nexacro._ChartGraphicsClassUtils.fmtNumber(this._translateY)];
	...
	_pGraphicsMatrix.rotate = function (angle, centerPt) {
		...
		angle = angle * nexacro._ChartGraphicsMatrix.DEGTORAD;
	...
	_pGraphicsMatrix.inverted = function () {
		var det = this._getDeterminant();
		return det && new nexacro._ChartGraphicsMatrix(this._scaleY / det, -this._shearY / det, -this._shearX / det, this._scaleX / det, (this._shearX * this._translateY - this._scaleY * this._translateX) / det, (this._shearY * this._translateX - this._scaleX * this._translateY) / det);
	};
...
if (!nexacro._ChartGraphicsQuadTree) {
	...
	_pGraphicsQuadTree._grow = function (xoffset, yoffset) {
		...
		var x = bounds.left - xoffset, y = bounds.top - yoffset, cls = nexacro._ChartGraphicsQuadTreeNode, rectCls = nexacro.Rect, w = bounds.width, h = bounds.height;
...
if (!nexacro._ChartGraphicsQuadTreeNode) {
	...
	_pGraphicsQuadTreeNode._createSubNodes = function () {
		...
		var top = bounds.top;
		var nodes = this._childNodes, cls = nexacro._ChartGraphicsQuadTreeNode, rectCls = nexacro.Rect;
...
if (!nexacro._ChartGraphicsRectsManager) {
	...
	_pGraphicsRectsManager.remove = function (rect) {
		var l = 0, i = 0, rects = this._rects, size = rects.length, r = size - 1, arrayCopy = nexacro._ChartGraphicsLibArray.arrayCopy, idx = 0;
	...
	_pGraphicsRectsManager._removeAll = function (rectsMgr) {
		var x, xChange = 0, rects = this._rects, size = rects.length, argRects = rectsMgr, i, j, argSize = argRects.length;
		var ret = false;
	...
	_pGraphicsRectsManager._retainAll = function (rectsMgr) {
		var x, xChange = 0, rects = this._rects, size = rects.length, argRects = rectsMgr._rects, i, j, argSize = argRects.length;
		var ret = false;
	...
	_pGraphicsRectsManager.append = function (rectsMgr) {
		...
		var rects = this._rects, dst = rects, size = rects.length, arrayCopy = nexacro._ChartGraphicsLibArray.arrayCopy;
	...
	_pGraphicsRectsManager.mergeRects = function (overhead, lineOverhead) {
		var rects = this._rects, i, j, size = rects.length;
...
if (!nexacro._ChartGraphicsObject) {
	...
	_pGraphicsObject.getGlobalBoundRect = function (bCopy, iscurrent) {
		...
		if (rect) {
			if (iscurrent) {
				var elems = [this], p = this.parent, pLayerCls = nexacro._ChartGraphicsLayer, m;
				while (p) {
					elems.unshift(p);
	...
	_pGraphicsObject._getTransform = function () {
		var m = this._matrix.clone(), xyM = new nexacro._ChartGraphicsMatrix();
	...
	_pGraphicsObject._getOwnerLayer = function () {
		...
		if (!ownerLayer) {
			var p = this.parent, pLayerCls = nexacro._ChartGraphicsLayer;
			while (p) {
				if (p instanceof pLayerCls) {
					ownerLayer = p;
					break;
					...
					if (fillGradationObj) {
						...
						if (gradationsubstyle == "linear") {
							...
							if (gradationpoint.centerX !== undefined) {
								ctxGradient = ctx.createRadialGradient(gradationpoint.centerX, gradationpoint.centerY, gradationpoint.innerRadius, gradationpoint.centerX, gradationpoint.centerY, gradationpoint.radius);
...
if (!nexacro.ChartGraphicsEllipse) {
	...
	_pGraphicsEllipse._drawShape = function (ctx, strokeMatrix) {
		...
		if (w == h) {
			...
			ctx.beginPath();
			var mx = w / 2, my = h / 2, kappa = nexacro._ChartGraphicsMathUtil._KAPPA, cx = mx * kappa, cy = my * kappa;
			if (strokeMatrix) {
...
if (!nexacro.ChartGraphicsPath) {
	...
	_pGraphicsPath.arcTo = function (pt_, radiusPt, rotation, clockwise, large) {
		...
		matrix = new nexacro._ChartGraphicsMatrix().translate(center).rotate(rotation).scale(rx, ry);
	...
	_pGraphicsPath.getPathData = function () {
		...
		var mpt = segs[0].point, fmtNumber = nexacro._ChartGraphicsClassUtils.fmtNumber;
	...
	_pGraphicsPath.setPathData = function (data) {
		...
		var parts = data.match(/[mlhvcsqtaz][^mlhvcsqtaz]*/ig), coords, relative = false, control, l, j, i, current = new nexacro.Point();
			...
			switch (lower) {
				...
				case 'a':
					for (j = 0; j < length; j += 7) {
						this.arcTo(current = getPoint(j + 5), new nexacro._ChartGraphicsSize(+coords[0], +coords[1]), +coords[2], +coords[4], +coords[3]);
					}
					break;
				case 'z':
					this.closePath();
	...
	_pGraphicsPath._drawShape = function (ctx, strokeMatrix) {
		var segs = this._segments, mpt = segs[0].point, coords, prevX, prevY, i = 0, l = 0;
		...
		function drawSegment_strokeMatrix (seg1, seg2, skipLine) {
			var point2 = seg2.point, controlPt1 = seg1.outVector, controlPt2 = seg2.inVector, pt;
	...
	_pGraphicsPath._getSegPathData = function (parts, seg1, seg2, skipLine) {
		var point2 = seg2.point, controlPt1 = seg1.outVector, controlPt2 = seg2.inVector, pt, fmtNumber = nexacro._ChartGraphicsClassUtils.fmtNumber;
		if (!(controlPt1 && controlPt2) || (controlPt1.isZero() && controlPt2.isZero())) {
			if (!skipLine) {
	...
	_pGraphicsPath._calcCurve = function (v0, v1, v2, v3, xyCoord, padding, min, max, roots) {
		var a = 3 * (v1 - v2) - v0 + v3, b = 2 * (v0 + v2) - 4 * v1, c = v1 - v0, count = nexacro._ChartGraphicsMathUtil.solveQuadratic(a, b, c, roots), tMin = this._curveMin, tMax = this._curveMax, u, t;
...
if (!nexacro.ChartGraphicsPaths) {
	...
	_pGraphicsPaths.destroy = function () {
		...
		if (this._currentFillgradation) {
			this._currentFillgradation = null;
		}
